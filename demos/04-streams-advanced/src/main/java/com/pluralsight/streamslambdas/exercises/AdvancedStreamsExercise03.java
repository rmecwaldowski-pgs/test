...
public class AdvancedStreamsExercise03 {
    ...
    public Map<PriceRange, List<Product>> groupProductsByPriceRange(List<Product> products, BigDecimal cheapLimit, BigDecimal expensiveLimit) {
        ...
        return products.stream().collect(Collectors.groupingBy(product ->
            product.getPrice().compareTo(cheapLimit) < 0 ?
                    PriceRange.CHEAP : (product.getPrice().compareTo(expensiveLimit) >= 0 ?
                    PriceRange.EXPENSIVE : PriceRange.MEDIUM)));
    }
}
